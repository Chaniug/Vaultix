 ...
 *
 * Credential Provider（M3 最高优先批次）：Android 14+ 系统层通行密钥供应方。
 *
 ...
 *
 * 行为：
 *  - onBeginGetCredentialRequest：按请求 rpId 匹配已解锁库的 passkey → 列 PublicKeyCredentialEntry；
 *    全部库锁定时，列一条「解锁 Vaultix」入口，其 PendingIntent 复用 AutofillActivity 解锁链
 *    （与自动填充下拉的解锁同款：用户解锁后回 Edge 重新触发，届时库已解锁）。
 *  - onBeginCreateCredentialRequest：列 CreateEntry → 引导用户在 Vaultix 内创建并绑定到登录条目。
 *  - onClearCredentialStateRequest：Vaultix 不维护登录态，直接成功。
 *
 * 密码填充仍由 VaultixAutofillService（AutofillService）承担（同源降级），本服务只做 public-key。
 */
...
class VaultixCredentialProviderService : CredentialProviderService() {
    ...
    private suspend fun buildGetResponse(request: BeginGetCredentialRequest): BeginGetCredentialResponse {
        ...
        if (options.isEmpty()) return BeginGetCredentialResponse.Builder().build()
        ...
        if (unlocked.isEmpty()) {
            ...
            entries += unlockEntry(options.first())
    ...
    /** 库锁定时的「解锁 Vaultix」入口：复用 AutofillActivity 解锁链。 */
    private fun unlockEntry(option: BeginGetPublicKeyCredentialOption): CredentialEntry {
        ...
        return PublicKeyCredentialEntry.Builder(
            ...
            .setDisplayName(getString(R.string.credential_unlock_subtitle))
            .setIcon(Icon.createWithResource(this, R.drawable.ic_passkey))
            .build()
    }
