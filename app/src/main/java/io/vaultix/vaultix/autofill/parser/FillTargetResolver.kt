 ...
 *
 * 只保留判定所必需的两条信息：本节点是否自己就能收值（`autofillType == AUTOFILL_TYPE_TEXT`）
 * 且带 `autofillId`，以及它的子节点。把 `ViewNode` 剥到这两个字段后，重定向算法就成了
 * 纯函数，可以直接喂给 JVM 单测 —— 而 `ViewNode` 是 `@SystemApi` 的抽象类，构造不出实例，
 * 真正的解析器（[AssistStructureParser]）在 JVM 上无从断言。
 *
 ...
 *
 * 剥成布尔值是为了可测：`AUTOFILL_TYPE_TEXT == 1` 而 `AUTOFILL_TYPE_NONE == 0`，
 * 二者不可互换 —— 后者表示「这是个容器 / 不可填控件」，往它上面 `setValue` 会被框架丢弃。
 *
 * ⚠️ 判据是 `autofillType`，**不是**「是不是输入控件」（[isEditableNode]：
 * `htmlInfo.tag == "input"` 或 className 是 EditText 家族）—— 这是错的：
