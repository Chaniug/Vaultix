...
## 🕐 最新状态（**2026-09-17 深夜（三）**·交接点，接力请先看这几行）

- **HEAD `47c71e2`；工作区干净。** 今天累计 **19 个提交**
  （R1–R4 → 通行密钥两层 → 签名统一 → 四处静默失败 → OneDrive 登录态 → ① 前三步 → 定稿三次修订）。
  ⚠️ `c97e2dd` 之前的 18 个**已推送**；`47c71e2`（定稿 §11.10–11.12）**未推送**。
...
- ★★ **下一步：三项已定稿、待施工**（规格自包含，见 `decisions/设置页信息架构-定稿.md`）：
  | 顺序 | 项 | 定稿 | 要点 |
  |---|---|---|---|
  | 1 | **「解锁方式」合并精简** | §11.11 | 逐库 × 两步 × N 次 → **一次流程 + 默认全勾**；⚠️「PIN 必须一次配齐」是**硬约束**，UI 须如实告知 |
  | 2 | **密码库管理页改「行 + ⋮」** | §11.10 | **不用卡片**（这是对 §11.1 的修订）；⋮ 里放 锁定/同步/退出/移除；状态放副标题 |
  | 3 | **SSH 条目「生成密钥对」** | §11.12 | ⚠️ 先定**私钥导出格式**：OpenSSH 新格式要自实现 bcrypt-kdf + chacha20；PKCS#8 近乎零成本。字段名须与 KeePassXC 逐字对齐 |
- ⏳ **还欠最后一次小活**（§11.9 第三条部分达成）：添加页在"**没有账号**"时仍落到原表单
  （服务器/密码/登录），应整段摘掉改给"去设置里配置"的引导。当前是**插入式**实现。
