...
- ★★ **下一步只有一件事**：按 **`Docs/progress/settings-rework.md`** 的
  **§1 → §2 → §3 → §4** 施工。该文档**自包含**（纪律 + 落点 + 验收清单，**零上下文可做**），
  设计依据 = `decisions/设置页信息架构-定稿.md` **§11.10 / §11.11 / §11.12**（先读那三节）。
...
- ★★ **待施工 = 三项 + 一处小活** —— **规格 / 落点 / 纪律 / 验收清单全在
  [`Docs/progress/settings-rework.md`](../Docs/progress/settings-rework.md)**（自包含；**本文件不复述**，
  以免两处漂移）：① 「解锁方式」合并精简 → ② 密码库管理页改「行 + ⋮」→ ③ SSH「生成密钥对」
  → ④ 添加页摘掉原表单。⚠️ ③ **要先定「私钥导出成哪种格式」**，否则会白写。
