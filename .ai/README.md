...
> 2. **编辑壳「去保护层」**：顶栏右侧动作按钮删掉 `Surface` 胶囊（透明填充 + 阴影合成出深色灰环）；
>    上下栏由「渐变保护层」改为**滚动后 alpha 硬切**（对齐列表页语义）。
>    并修掉一处**静默失效** —— 系统栏逻辑（窗口背景/导航遮罩/图标深浅）**整段从未执行**，
>    因为 `LocalView.current` 取在了 `Dialog { }` **外面**（拿到的是 Activity 的 view）。
> 3. **条目选择器按 RP 自动匹配预选 + 可搜索**（用户反馈「还得自己滑动查找」），对齐 Bitwarden 手感。
>
> ⚠️ **当前位置：以上均已真机验证；`main` 已推送到 `119d3a1`。**

- **工作区干净；今天的提交已全部推送**（`main` 在 `119d3a1`；**`rele` 未动**，仍是 `0ab342f`）。
  今日链条：编辑壳去保护层 → 顶栏去胶囊 → 状态栏图标深浅 → 真机装机核证 →
  通行密钥 create 响应补字段 → 选择器自动匹配。
