...
## 🕐 最新状态（**2026-09-21 收工**·交接点，接力请先看这几行）

> ### ★ 本轮（2026-09-21）做了什么 —— 四句话
> 1. **通行密钥注册链路打通**（两个真机根因、**两层不同解析者**）：#108 修的是 **RP 层**解析
>    `attestationObject`（CBOR 键名要 `attStmt`）；#109 修的是 **浏览器层**解析 CredMan 响应
>    （create 响应的 `response` 缺 `publicKeyAlgorithm` ⇒ Chromium **整份拒收**，根本到不了 RP）。
>    两层缺一不可。验收：`MojoClassFromJSON` 报错 **2 次 → 0 次**，用户确认注册成功。
> 2. **编辑壳「去保护层」**：顶栏动作按钮删掉 `Surface` 胶囊；上下栏由「渐变保护层」改为
>    **滚动后 alpha 硬切**。并修掉一处**静默失效** —— 系统栏逻辑（窗口背景/导航遮罩/图标深浅）
>    **整段从未执行**，因为 `LocalView.current` 取在了 `Dialog { }` **外面**（见 #110）。
> 3. **施工单 5 项全部落地**（[`../Docs/progress/totp-ux-and-dev-diagnostics.md`](../Docs/progress/totp-ux-and-dev-diagnostics.md)）：
>    验证码页点标题隐藏（跨重启保持）/ 临期换码开关 / 关于页 / **debug 开发者模式（查看+导出日志）**/
>    日志取舍（**只写 logcat、不写文件** ⇒ release 磁盘占用恒 0）。施工中发现并修了一处
>    **谎报成功**（见 #111）。
> 4. **Actions 缓存**：新增 `cache-cleanup.yml`（每周一自动 + 可手动触发）。
>
> ⚠️ **当前位置：`main` 已推送（见末尾提交）；`rele` 未动**。
> **发版（`main`→`rele`）待用户决定 `VERSION` 是否从 `0.5.0` bump 到 `0.6.0`**（本轮改动量大）。

- **工作区干净；今天的提交已全部推送**（`main` 见当日末尾；**`rele` 未动**，仍是 `0ab342f`）。
  今日链条：编辑壳去保护层 → 顶栏去胶囊 → 状态栏图标深浅 → 真机装机核证 →
  通行密钥 create 响应补字段 → 选择器自动匹配 → 施工单 5 项落地（隐藏 / 开关+谎报修复 /
  关于页 / 开发者模式）→ 关于页二次精简 → 缓存清理工作流。
- ★★ **当前最该记住的三条纪律**（都付过代价）：
  1. **写"为什么不是 X"必须绑定到具体的解析者/层** —— #108 漏了这一句，害后人绕了一整轮；
  2. `?: return@Effect` 这类"拿不到就静默返回"会把整段功能变成**不报错的死代码** ——
     排查顺序是**先确认"有没有执行"，再问"执行了为什么不对"**；
  3. **"提示成功"的代码必须与"做事"的代码写在同一条路径上** —— 只提示不做事 = 谎报。
- ⚠️ **真机验收未做**：本轮全部 UI 改动只过了编译/detekt/单测。优先验
  「设置 → 开发者 → 查看与导出日志」的**分享**（FileProvider 是新增基建，报错不指向根因）。
- ⚠️ **GitHub Actions 缓存仍贴上限**（约 9.9 / 10 GB）：占用由**每次 push 新增约 300 MB** 驱动
  （12 + 240 + 51 MiB 三块切片），不是陈年堆积 ⇒ 想压下来要从"每次构建少产出缓存"下手。
...
| `tools/` | **本地自检脚本**（不需 Android SDK，`python3` 直接跑）。覆盖「编译器才能发现、detekt 查不到」的缝：<br>`check_compile_smells.py` = 图标导入 / 顶层常量顺序 / `R.string` 悬空引用 / **重复声明**（第四个检查，见 #103；带 `--detekt-probe` 两级探针）；<br>`check_signature_types.py` = 签名里的类型名是否存在；<br>`check_import_packages.py` = `import` 的包路径对不对（#101 / #104）；<br>`check_experimental_optin.py` = 实验性 API 有没有 `@OptIn`（#101.2，带 `--selftest`）；<br>`check_state_flattening.py` = **多态状态（sealed ≥3 分支）有没有被压成二值开关**（#129；`Switch(checked = x is S.On)` 会把 `Partial` 与 `Off` 合并 —— `is On` 是合法 Kotlin，类型检查与 detekt **双双查不出**）；<br>`fetch_release_asset.sh` = **分块断点续传下载 release 资产**（沙箱整文件下载不可靠，见 #105；自带 `zipfile` 完整性校验） |
| `tools/tests/` | **门禁自己的自测**（改探针前必跑）：<br>`selftest_duplicate_declarations.py` = 7 个正反用例；<br>`selftest_import_packages.py` = **端到端**（真把 import 改错、跑脚本、看退出码、还原）；<br>`selftest_cross_file_private.py` = **端到端**（真把 `internal` 改回 `private`、验证报出两个调用方、还原）；<br>`selftest_state_flattening.py` = 9 个正反用例。⚠️ 含**全限定名**形态（`Outer.Cap.On`）—— 少了它，探针会在真实代码上静默失效（见该文件开头） |
