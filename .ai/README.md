...
## 🕐 最新状态（**2026-09-17 深夜（二）**，接力请先看这几行）

- **HEAD `72d9b96`；工作区干净。** 今天累计 **11 个提交**（R1–R4 → 通行密钥两层 → 签名统一
  → 四处静默失败 → ① 的第一刀）。
- ★★ **今天最大的收获是一条共同病根**：**某个分支/某处状态悄悄丢失，而现象与真因之间
  没有任何可观测的连接。** 它有三种表现形态，今天全踩过：
  | 形态 | 例子 |
  |---|---|
  | **沉默的分支**（不改状态、不留日志） | 通行密钥 `Ready` 分支；同步取消分支不清 `isRunning` |
  | **假空态**（把失败渲染成空） | 读路径解密失败 → 空列表（用户以为密码丢了） |
  | **假未登录**（长寿命状态存在短寿命页面里） | 「添加密码库」返回即销毁 VM ⇒ 登录态"返回就没" |
  ⇒ 三条约定（见 `SESSION-2026-09-17.md` §13.1）：**要么改状态要么留日志** ·
  **「空」有三态（加载中/真的没有/查询或解密失败）** · **状态的生命周期不能短于页面的**。
- **进行中：① 密码库管理重构**（设计定稿 `decisions/设置页信息架构-定稿.md` **§11**，含 §11.7 施工顺序、§11.8 实施记录）
  - ✅ 已完成：`CloudAccountInventory`（账号清单**从库 origin 反推**，不另存一份）·
    `CloudAccountsScreen/ViewModel`（设置 → 密码库管理 → **网盘账号**）· 路由与入口接线。
  - ⏳ **下一步（§11.8 末尾）**：把「添加密码库 → 从网盘添加」的**登录/凭据去掉**，改成
    "**选一个已配置账号 → 列目录 → 选文件**"。⚠️ 这是**流程重设计**（要动 VM 的账号数据流），
    不是删几行 UI；**做完它，今天那个"返回就丢登录态"才算真正修掉**。
  - ⏳ 再之后：密码库管理页卡片化 + 「锁定/退出/移除」按库分级（§11.1/§11.2）；备份 ≤3 份。
- ⏳ **待装机**：最后一次 `adb install` 被拒（**设备锁屏 + Dozing**，项目里记过这个坑）；
  解锁后重跑即可。
- ⚠️ **未结**：`decodeTrashRows` 同款"静默跳过" · B 决策文档（模块边界） ·
  「格式中转站 / 灾备三层」的判断（§13.4，尚未成文）· R1–R3 的三条真机验收。
- ⚠️ **诊断手法（今天验证有效，可复用）**：`run-as` + **`exec-out`** 拉
  `databases/vaultix.db`（**必须含 `-wal`/`-shm`**）→ 本地 sqlite3 查行数/归属/密文长度；
  MSAL 的账户在 `shared_prefs/com.microsoft.identity.client.account_credential_cache.xml`。
  ⚠️ Room 表列名是**驼峰**（`vaultId`/`deletedDate`）；`adb pull` 目标必须给 **Windows 路径**；
  大日志用 `exec-out tail -c 2500000` 只取尾部；**python 是 Windows 版，不认 `/d/...` 路径**。
- ⚠️ **门禁必须含 `test`**，不能只跑 `compile`（`SESSION §6.1` 的教训）。
