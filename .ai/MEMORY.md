...
> **接力顺序（先读这四份，再动手）**：
> 本文件（约定 / 硬约束 / 决策）→ `.ai/ISSUES.md`（坑：现象-根因-解法，最新 **#126**）→
...
| M2 | KDBX 引擎 + 集成 | 🔄 **阶段 A（只读）完成**；**阶段 B（整库写回）已可用**（`Kdbx.save`/`saveVia` 正被网盘同步真调用）；**条目级编辑被闸住** ⇒ 下一轮 R-6…R-9 开放 |
...
> 本轮只做前半（修坏的），后半 R-6…R-9 是下一轮起点。

**推送 `d17772b → f160ace`（本轮 6 个提交）· 门禁：detekt 净 18→17 ✅ / 类型检查 ✅ /
两个新探针各自做**变异 + 闭环**验证 ✅**（⚠️ 沙箱无 Android SDK，未跑 Gradle 全量编译）

1. ★★ **开关「不猜状态」只修了一半（`ISSUES.md` #120，`06bd5a4`）** ——
   同一个设置页上**两种开关行为并存**：3 个用灰占位（点了才变）、7 个用裸 `Switch`（首帧直接 `true`）。
   **根因在类型层**：`StateFlow<Boolean>` 不允许 null ⇒ 磁盘没读出来时只能猜一个，
   `initialValue = true` 就把「偏好还没读出来」**伪装成「明确开着」**，
   违背项目自己 `SettingsScreen.kt:1316-1329` 的「不认识就什么都不说」。
   修法：8 个流 → **`StateFlow<Boolean?>`**、初值 `null`；`SettingsSwitch` 收口为唯一实现（`internal`）。
   ⚠️ **泄漏点 `FillBehaviorSection`**：`Boolean?` 到了这一层**又收窄回 `Boolean`** ——
   **收窄处编译不报**（把 `Boolean?` 传进 `Boolean` 参数才报，反过来是隐式的）。
   > **判据：类型改造最容易死在"中途某一层收窄"上。**
2. ★★ **快解三态开关失联（#121，`561bf11`）**：`checked = x is CapabilityState.On` 把
   三态压成二值 —— `Partial`（「已开启但只覆盖一部分应用」）被渲染成**关着的开关**，
   用户点它是"打开"还是"补齐"**无从判断**。类型检查、detekt、`when` 穷尽性**全都查不出**
   （压扁发生在"类型 → 界面"的缝里）。修法：拆成**两种控件** ——
   `Partial` → **「继续」按钮**，`On`/`Off` → `Switch`。
   > ★ **可复用判据（#122）**：**当注释不得不解释「点这个按钮是 A 不是 B」时，
   > 错的是控件，不是注释。**
3. ★★ **两个新探针，且两个都"坏过"**（#124 / #126）：
   - `check_state_flattening.py` —— **连坏两版都是假绿**：① 只扫当前文件的 sealed 声明
     （真 bug 的声明在另一个文件里）② 正则贪婪吃到倒数第二段（`…CapabilityState`）⇒ 仍报 0。
     **自测全绿但真代码上报 0** —— 因为我所有用例都写短名（`Cap.On`），抓不到自己这个 bug。
     > ★ **探针的闭环证明 = 「把代码还原成出 bug 的版本，看它是否恰好报那几行」**，
     > **不是「自测全绿」。** 自测只证明"探针能处理我编的简化输入"。
   - `check_orphan_strings.py` —— 834 条里 **126 条零引用**。**是报告器不是删除器**：
     按「被搬走的留，被取代的删」，留档的字符串**本身就是零引用的** ⇒ 这个数**不可能归零**。
     方向性取舍：**宁可少报几条孤儿，也不能误删在用的**（误报编译期可见，误删运行时才炸）。
4. ★★ **勘误：detekt 参数上限是 7 个，不是 8 个（#123）** ——
   `LongParameterList.functionThreshold = 8` 的判定是**「参数个数 ≥ 阈值」**，
   **8 个就红**。"阈值"是**触发点**，不是上限。
   项目里 `FormValues.kt:42`、`TotpCodesScreen.kt:672` 两处注释写的「8 个刚好」**是错的**。
   ⟹ **后果**：`ItemRepositoryImpl` 已 **10 参** ⇒ 给它加构造参数不可行 ⇒
   **KDBX 写入必须抽独立协作者类**（下一轮 R-7 的前提）。
5. **副标题截断 + 删冗余参数（#123，`da60613`）**：`SettingsRow` 副标题不截断 ⇒
   长文案（60+ 字）撑高整行、破坏"一组一行"的分组感 ⇒ `maxLines = 2` + `Ellipsis`；
   删全仓零调用的 `showSubtitle` ⇒ **8 参 → 7 参**，白捡一处 detekt 违例（等路径对比确认）。
6. **一条文案 + 孤儿串基线（#126，`d98b4f0`）**：`setting_autofill_save_prompt`
   「保存提示」→「**登录后询问保存**」。⚠️ 我上一轮说「有 10 条文案模糊」**是错的**，
   逐条审完只有 1 条真该改（`setting_fill_assist_desc` **刻意对齐 Bitwarden 官方 zh-rCN 措辞**，
   **对齐上游的文案不叫模糊，叫保持一致**）。
7. ★★ **账目体检：7 篇里 6 篇计数是错的（`f160ace`）** ——
   实测 01→32(原27) 02→18(原19) 03→18(原15) 04→8(名单漏#88) **05→10(原3)** 06→32(原22) 07→6(✅)。
   更根本：此前**只有 06 篇有题注行**，其余六篇压根没有 ⟹「看数字判断进度」多数分篇上不成立。
   `.ai/ISSUES.md` 里 09-21 写的补救是「下次改动某一篇时顺手改正该篇的数字」，**实际只改了 02** ——
   > ★ **教训不是"要更勤快"，是"两份事实必然分叉"**：
   > **「顺手」对不上 —— 改动者只碰自己那篇，看不见别人那篇的数字。**
   已加三重互校脚本（数字/名单/正文 + 索引表交叉核对），全部 ✓。
   ⚠️ **顺带三处"引用与正文各自演化"**：`#83` 被 02 篇名单引用但**全仓无正文**；
   `.ai/README.md` 把两个新探针标成 `#132`/`#129`（实际 `#126`/`#124`）—— 均已修。

**第八十轮前后（2026-09-17 凌晨 · 见 `.ai/SESSION-2026-09-17.md`）**
...
0. ⭐⭐ **开放 KDBX 单条编辑（R-6 … R-9）—— 用户已拍板，是下一轮的正题。**
   方案见 `.ai/SESSION-2026-09-26.md` §8。**前置事实（已核实，别重新论证）**：
   KDBX **整库写回早就能用了**（`Kdbx.save` / `saveVia` 正被网盘同步真调用）；
   被闸住的**只是条目级编辑**（`requireWritable` 抛 `ReadOnlyVaultException`，#106 的 `8fc1125`）
   ⟹ **开放编辑不是"撤闸门"，是新建一条独立写路径。**
   ⚠️ **铁律**（`KdbxEncoder.kt:14-21`）：必须**克隆原 `Entry` 再逐字段覆盖** ——
   走「领域模型 → 重建库」必丢所有未映射的东西（`KPEX_*`、未知自定义字段、附件、历史、自定义图标）。
   ⚠️ **R-6 有前置阻塞项**：kotpass `Entry`/`EntryFields` 的**可写 API 形态**需在有网环境
   跑 `./gradlew :data:kdbx:compileKotlin` 核对（沙箱无依赖缓存）。
1. ⭐ **应用内 PIN 解锁**（用户连续三轮要求，一直欠着）。
   定位 = **「解锁便利」而非找回手段**：PIN 只用于解开 Keystore 里已包好的那份密钥
   （与指纹**同一条链**），**不参与派生库密钥**；需要设置页开启/修改/关闭 +
   解锁页数字键盘入口 + 主密码兜底。
2. **搬运 Bastion 备份体系**（自动备份 / 保留策略 / 备份内容选择 / 备份报告）——
   本轮用户明确划到下一轮。
3. ✅ ~~性能专项 P0「先换 release 包建基线」~~ **已完成**（第五十三轮）。
...
4. **126 条孤儿字符串的逐条判读** —— 必须人对「被搬走的留 / 被取代的删」逐条决定；
   **不可自动化**（会毁掉刻意留档的对照文案，且没有编译错误会拦住它）。
5. **修 #125**：`check_import_packages.py` 把 `Icons.Filled.Build` 误判成 `android.os.Build`。
   ⚠️ **别顺手把 `Build` 加进豁免表** —— 那会把真的 `android.os.Build` 误用一起放行。
6. **`ItemsViewModel.showIcon`** 仍是 `StateFlow<Boolean>`（`SharingStarted.Eagerly`，
   线程穿过 `ItemsScreen.kt` 一堆 `Boolean` 参数）—— 那属于**卡片渲染**不是设置开关，
   R-3 未覆盖，可择机收口。
7. ⏳ **主题图标（monochrome）未验**：需用户在系统里开启「主题图标」才能看 V 挖空的效果，
   **未擅自改用户系统设置**。开启后可截图确认。
8. 待用户复测：验证码页冷启动的「假空态」是否已变成转圈（#84）。
9. 同类「假空态」尚存于 `ItemsScreen:310` 与卡包页；密码页因活跃库 id **同步**解析暂未复现。
10. `deliverPendingFill()` 拿不到有效暂存时是**静默** `finish()`，值得补一条"填充已失效"提示。
11. **密保问题：结论是不做** —— 它不是找回手段，只会给同一个库再加一把更弱的钥匙。

**门禁（本地真跑全绿才提交）**：全模块 `detekt` → `:app:compileFullDebugKotlin` →
`:app:assembleFullDebug` → 用发布密钥重签 → `adb install -r` → 真机验收。
⚠️ **沙箱内拿不到 Android SDK 时**：`.ai/tools/` 六道脚本 + Kotlin 类型检查 harness
（`/tmp/kcheck.sh`，见 `SESSION-2026-09-26.md` §6）+ **detekt 用 `/tmp/detekt-1x.yml`**。
⚠️ 新增探针**一律先做变异测试 + 还原成 bug 版本的闭环证明** —— 历史上已出现 **4 种"探针假绿"**。
...
> **踩坑全表**（现象 → 根因 → 解法，编号 #1~**#126**，索引见 `.ai/ISSUES.md`）：
> ⚠️ **条数别信记忆** —— 2026-09-26 体检发现**7 篇里 6 篇的计数都是错的**（见 §9 本轮第 7 条）。
...
> 2026-09-14 起的轮次请直接看 §9 顶部与对应的 SESSION 文件；最近四轮是：
...
>   + OneDrive 鉴权骨架**。
> - `SESSION-2026-09-21.md` —— **通行密钥注册链路打通（两层解析者 #108/#109）**
>   + 编辑壳去保护层 + 施工单 5 项落地。
> - `SESSION-2026-09-26.md` —— **设置页缺陷修复（开关「不猜状态」#120、快解三态开关 #121）
>   + 两个新探针（都坏过 #124/#126）+ detekt 阈值勘误 #123 + 七篇计数校正**。
>   **下一轮起点 = 开放 KDBX 单条编辑（R-6…R-9）**，该文件 §7 是完整交接。
