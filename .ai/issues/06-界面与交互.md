...
## 107. 全屏编辑壳上下两条**满宽不透明**栏 ⇒ 深色 / OLED 下就是两条黑带（2026-09-20，`FullScreenDialogShell`）
...
### 根因：把"沉浸"实现成了"满宽栏 + 淡出"，而这两者都难看

沉浸感的来源**不是**"栏透明"，而是**"内容从栏的外侧穿过"**。
满宽栏无论透明与否都不满足这一条：不透明时是色带，透明时是叠字。

### 三方案对比（选①）

| # | 方案 | 判定 | 为什么 |
|---|---|---|---|
| ① | **悬浮胶囊**（圆角 50% + 四周留白 + 恒不透明） | ✅ **采用** | 内容从胶囊**外侧**穿过（不经过胶囊本体）⇒ 既不叠字又有清晰层次；与列表页底栏 `VaultixBottomDock` **完全同规格** |
| ② | 毛玻璃 / `Modifier.blur` | ❌ | `blur` 在 minSdk 26 的 **API 31- 是 no-op**（设备分叉）；离屏渲染**掉帧耗电**；且 OLED 下模糊纯黑**仍是纯黑** —— 花的代价一点没解决问题 |
| ③ | 半透明底色（`copy(alpha = …)`） | ❌ | 直接踩 `8.4`「**容器色阶嵌套必须换档，禁止同色叠 alpha**」（那正是"卡片边界消失、整屏糊成灰"的成因） |

### 落地要点

- 两栏改成 `Surface` + `RoundedCornerShape(50)` + `surfaceContainerHigh` + `tonal 3dp` +
  `shadow 6dp`（**与列表页底栏同值**，见 `8.4`「一屏里出现两套规格 = 观感廉价的首要来源」）。
- ⚠️ **`shadowElevation` 必须恒定**（写死常量，不来自 `animateXxxAsState`）——
  阴影几何参与动画会**重建渲染层**、掉帧。这条已在 `VaultixExpressiveTopBar` 的
  `PILL_SHADOW` 上踩过（`SESSION-2026-09-20.md` §6.1）。
- ⚠️ `statusBarsPadding()` / `navigationBarsPadding()` 排在胶囊**外层** —— 胶囊因此落在
  状态栏**下方**、四周留白露出内容。这是**有意的行为改变**：前两轮"满宽栏覆盖状态栏"，
  胶囊要"浮"就不能吞掉整条状态栏。状态栏图标仍按 `surface.luminance()` 定色（见该文件 KDoc）。
- **滚动淡出整套逻辑删掉**（`barAlpha` / 判定阈值 / `tween`）：栏外观与滚动位置**无关**，
  与列表页底栏一致。
- ⇒ `contentClearance` 的**公式不变**，但 `barHeight` 的口径从"栏体高"升为
  "**栏体高 + 胶囊留白**"（`TITLE_BAR_OCCUPIED = 48+8+12 = 68dp`、
  `ACTION_BAR_OCCUPIED = 72+12 = 84dp`）。见 `8.4` 的补充条。

### 与 `SESSION-2026-09-20.md` §5.3 的关系（★ 那条已被本轮推翻）

§5.3 写过"底部条**有意不做**胶囊，因为必须一直可点"。**本轮复核结论：那条理由不成立** ——

> `SESSION-2026-09-20.md` §5.3 原话：「底部必须一直可点 ⇒ 只做到'淡出底色'这一档，**不**改成胶囊。」

- **胶囊同样一直可点**：列表页的 `VaultixBottomDock` 就是胶囊，且恒在**最上层**、从不遮挡点击。
- §5.3 真正要防的是"按钮做成**半透明**、看不出能点"；
- 胶囊**不透明** ⇒ 在不牺牲可点性的前提下消除了上下两条黑带。
- ⇒ §5.3 的结论据此更正（保留原文 + 标注，见该文件）。

### 复现判据（下次遇到同类"黑条"直接照这个查）

> 全屏壳 / 悬浮元素的"黑条"问题，先问三个问题：
> ① 栏是否**满宽**？② 栏底色是否与**页面**同色？③ 深色 / **OLED 纯黑**下是否放大？
> 三问全中 ⇒ 不是"颜色不对"，是**形状 + 相邻关系**不对，改形状（缩体积 + 留白）而不是调色。
