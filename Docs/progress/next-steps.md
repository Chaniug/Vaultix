...
>
> ### ✅ 已实现（2026-09-13 23:55，待你对视觉拍板）
>
> **设计**：盾牌 + 钥匙孔（把原来的"盾"与旧前景的"锁"合成一个更准的符号：
> 盾 = 安全，钥匙孔 = 密码库）。白图形 + 蓝渐变底（品牌蓝 `#3B5BDB` 派生 `#4C6BE8 → #2438A0`）。
>
> **落地的文件**：
> | 文件 | 作用 |
> |---|---|
> | `drawable/ic_launcher_background.xml` | 满幅 108×108 渐变底板（**不内缩**，任何遮罩都有色） |
> | `drawable/ic_launcher_foreground.xml` | 图形本体，`fillType="evenOdd"` 挖出钥匙孔 |
> | `drawable/ic_launcher_monochrome.xml` | 单色层（API 33+ 主题图标；复用同一条路径，保证形状一致） |
> | `mipmap-anydpi-v26/ic_launcher.xml` | adaptive-icon 声明（background + foreground + monochrome） |
> | `values/colors.xml` | 新增 `ic_launcher_bg_top` / `ic_launcher_bg_bottom`（原 `ic_launcher_background` 保留） |
> | ~~`drawable/ic_launcher.xml`~~ | **已删除**（旧占位图，manifest 改指后成为死文件） |
> | `AndroidManifest.xml:43` | `@drawable/ic_launcher` → **`@mipmap/ic_launcher`**（这一步是整件事的关键） |
>
> **几何自校验**（脚本采样曲线求离画布中心的最大距离）：
> 盾形最远点 **30.00dp ≤ 33dp**（安全圈半径），留 3dp 余量 ⇒ 圆/squircle/水滴等任何遮罩都不切图形。
> ⚠️ 钥匙孔必须是**单条不重叠轮廓**（圆与锥形槽求并集，接点 P_r=58.45,53.02 / P_l=49.55,53.02）：
> 若用"圆 + 槽"两条子路径叠加，`evenOdd` 会把重叠区重新填上、边缘留一道接缝。
>
> **验证**：① 五遮罩预览（圆形/圆角方/squircle/水滴/直角方）图形均完整；
> ② `:app:assembleFullDebug` 通过；③ **真机覆盖安装后截图确认**：新图标填满 Honor 的圆角方遮罩、
> 盾与钥匙孔清晰完整（对比改前是"小盾牌 + 一圈留白"）。
